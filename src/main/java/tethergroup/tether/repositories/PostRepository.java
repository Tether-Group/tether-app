...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(nativeQuery = true,
            value = "select * from posts p order by p.post_date desc")
    List<Post> getAllPostsOrderedByPostDateDesc();

    List<Post> findByGroup_IdOrderByPostDateDesc(Long groupId);

    @Query(nativeQuery = true,
            value = "SELECT DISTINCT p.* FROM posts p INNER JOIN users u ON u.id = p.user_id INNER JOIN memberships m ON m.user_id = u.id INNER JOIN `groupstable` g ON g.id = m.group_id WHERE u.id = :user_id")
